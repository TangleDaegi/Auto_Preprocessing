...
def PPT_KC_Action():
    ...
    for file in files:
        PPT_Study.PPT_KC(file,df.Detail[0],df.Detail[1],dff.Detail[0])
...
def UFontAc():
    files = [f for f in os.listdir(df.Detail[0]) if re.match('.*[.]ppt', f)]
    for file in files:
        PPT_Font.AutoFont(file, df.Detail[0], df.Detail[1],
...
def U2FontAc():
    files = [f for f in os.listdir(df.Detail[0]) if re.match('.*[.]ppt', f)]
    for file in files:
        PPT_Win.AutoFont2(file, df.Detail[0], df.Detail[1],
                          select.get(select.curselection()))
    messagebox.showinfo(title="Finish", message="Job Finished!")
    return
...
def GTrans2EnAc():
    files = [f for f in os.listdir(df.Detail[0]) if re.match('.*[.]ppt', f)]
    for file in files:
        PPT_Trans.GTrans(file, df.Detail[0], df.Detail[1], 'en')
    messagebox.showinfo(title="Finish", message="Job Finished!")
    return
...
def GTrans2KoAc():
    files = [f for f in os.listdir(df.Detail[0]) if re.match('.*[.]ppt', f)]
    for file in files:
        PPT_Trans.GTrans(file, df.Detail[0], df.Detail[1], 'ko')
    messagebox.showinfo(title="Finish", message="Job Finished!")
    return

#Folder에서 File List 만들기
def Folder_FileList():
    FileList.FileList(df.Detail[2],df.Detail[1])
    messagebox.showinfo(title="Finish", message="Job Finished!")
    return
